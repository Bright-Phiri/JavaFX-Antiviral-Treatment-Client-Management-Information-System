...
public class statisticsController implements Initializable {
    ...
    String[] reportType = {"All Clients", "New Clients", "Returning Clients", "Doctors"};
    @FXML
    ...
    private void setChartData() {
        ...
        try {
            ...
            newclients = count1;
            data.addAll(new PieChart.Data("All Users", allUsers),
                    new PieChart.Data("Clients", allClients),
                    new PieChart.Data("Doctors", allDoctors),
            ...
            series.getData().add(new XYChart.Data<>("All Users", allUsers));
            series.getData().add(new XYChart.Data<>("Clients", allClients));
            series.getData().add(new XYChart.Data<>("Doctors", allDoctors));
    ...
    private void exportToPdf(ActionEvent event) {
        ...
        switch (reporttype) {
            ...
            case "Doctors": {
                Thread thread = new Thread(new GenerateReport1());
                thread.setDaemon(true);
                thread.start();
                break;
            }
    ...
    private void exportToExcel(ActionEvent event) {
        ...
        switch (reporttype) {
            ...
            case "Doctors": {
                try {
                    connection = Database.connect();
                    preparedStatement = connection.prepareStatement("SELECT count(*) FROM User WHERE Role = 'Doctor'");
                    resultSet = preparedStatement.executeQuery();
                    resultSet.next();
                    int numOfRows = resultSet.getInt(1);
                    if (numOfRows == 1) {
                        ...
                        try {
                            preparedStatement.clearParameters();
                            resultSet.close();
                            preparedStatement = connection.prepareStatement("SELECT * FROM User WHERE Role = 'Doctor'");
                            resultSet = preparedStatement.executeQuery();
                            Task<Void> task = new Task<Void>() {
                                @Override
                                protected Void call() throws Exception {
                                    ...
                                    File userReport = new File(System.getProperty("user.home") + "\\Documents\\System Reports\\Doctors.xlsx");
                                    FileOutputStream fileOutputStream = new FileOutputStream(userReport);
                                    workbook.write(fileOutputStream);
                                    fileOutputStream.close();
                                    Platform.runLater(() -> {
    ...
    private class GenerateReport1 extends Task<Void> {

        @Override
        protected Void call() throws Exception {
            SaveReport report = new SaveReport("SELECT COUNT(*) FROM User WHERE Role = 'Doctor'", "users", "users");
            report.saveReport();
            return null;
        }
    }
